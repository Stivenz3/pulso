...
export default function ProgressPage() {
  ...
  const { habits, activeHabitId, triggers, moods } = useAppStore();
  ...
  const totalClean = activeHabit?.cleanDaysTotal ?? currentStreak;
  const relapsesAvoided = activeHabit?.relapsesAvoided ?? 0;

  const recentTriggers = triggers.slice(0, 5).map((t) => t.type);
  const recentMoods = moods.slice(0, 5).map((m) => `${m.mood}(${m.intensity})`);
  ...
  // Streak chart data — weekly buckets based on current streak
  ...
  const relapseData = [
    { month: "Mes 1", relapses: 0 },
    { month: "Mes 2", relapses: 0 },
    { month: "Mes 3", relapses: 0 },
  ];
      ...
      >
        ...
        <div>
          ...
          <p className="text-[#908fa3] text-sm mt-2">
            Crea un hábito para empezar a ver tu progreso.
          </p>
        </div>
        <button
    ...
    >
      ...
      <section className="grid grid-cols-2 gap-4">
        {[
          ...
          { icon: Shield, label: "Recaídas evitadas", value: String(relapsesAvoided), color: "#ffb4a1" },
        ].map((item, i) => (
          <motion.div
            key={item.label}
        ...
        >
          ...
          <div className="relative z-10">
            <h3 className="font-[Space_Grotesk] text-xl font-bold text-white mb-2">
              Proyección de Impulso
            </h3>
            <p className="font-[Manrope] text-sm text-white/70 mb-5">
              Si mantienes este ritmo, estás en camino a tu próximo hito.
            </p>
        ...
        <div className="h-44">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              <XAxis
                dataKey="week"
                tick={{ fill: "#908fa3", fontSize: 10, fontFamily: "Manrope" }}
                axisLine={false}
                tickLine={false}
              />
              <YAxis hide />
      ...
      >
        <div>
          ...
          <p className="text-xs text-[#908fa3]">
            {relapseData.every((d) => d.relapses === 0)
              ? "Sin recaídas registradas — excelente 🎉"
              : "Tendencia registrada"}
          </p>
        </div>
        <div className="h-36">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={relapseData} barSize={28}>
              <XAxis
                dataKey="month"
                tick={{ fill: "#908fa3", fontSize: 10, fontFamily: "Manrope" }}
                axisLine={false}
                ...
                content={({ active, payload }) =>
                  active && payload?.length ? (
                    <div className="bg-[#2a292e] border border-white/10 rounded-xl px-3 py-2 text-xs text-white font-bold">
                      {payload[0].value} recaídas
                    </div>
                  ) : null
                }
              />
              <Bar dataKey="relapses" radius={[6, 6, 0, 0]}>
                {relapseData.map((entry, index) => (
                  <Cell
                    key={index}
                    fill={entry.relapses === 0 ? "#3832f6" : "#a82700"}
                    fillOpacity={0.8}
                  />
                ))}
              </Bar>
      ...
      {triggers.length > 0 && (
        ...
        >
          <div className="p-5 border-b border-white/5">
            ...
            <p className="text-xs text-[#908fa3]">Últimos triggers registrados</p>
          </div>
      ...
      {correlation && (
        ...
        >
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#3832f6] mb-2">
            IA · Correlación Emocional
          </p>
